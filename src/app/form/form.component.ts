...
export class FormComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe(params => {
      ...
      if (userId) {
        ...
        this.loadUserData(userId); // Load user details for editing
      } else {
        this.resetForm(); // New user, reset form
      }
    });
    // this.loadUsers();
  }
  ...
  loadUserData(userId: string): void {
    this.http.get<any>(`http://localhost:3000/customer/${userId}`).subscribe(
      (user) => {
        this.user = { ...user, id: user._id, stays: user.stays || [{ checkIn: '', checkOut: '' }] };
      },
      (error) => {
